...
class PostProcessor:
    ...
    def plot_scaling(self, plot_name: str, df_scaling: pd.DataFrame = None):
        ...
        core_processes = {col.split('_')[2]: col for col in df_scaling.columns}  # Mapping core name to column
        unique_processes = sorted(set(core_processes.keys()))  # Unique process names (without prefixes)
        colormap = plt.colormaps['tab20']  # A palette with more unique colors
        color_map = {process: colormap(i / len(unique_processes)) for i, process in enumerate(unique_processes)}
        categories = [f[:4] for f in self.m.FUNCTIONAL_FLOW]  # Extract the first four letters of each functional flow
        ...
        for ax, category in zip(axes, categories):
            ...
            columns = [col for col in df_scaling.columns if col.startswith(f"{category}_prod_")]
            ...
            ax.set_title(f"Scaling Factors for {category.capitalize()} Production", fontsize=14, pad=10)
            ...
            simplified_labels = [process.split('_', 2)[-1].replace('_', ' ').capitalize() for process in columns]
            ax.legend(
                handles,
                simplified_labels,
                title="Process Type",
                ...
                for t in self.m.SYSTEM_TIME:
                    ...
                    for tau in self.m.PROCESS_TIME:
                        if t - tau in self.m.SYSTEM_TIME:
                            total_production += self.m.foreground_production[p, f, tau] * self.m.scaling[p, t - tau].value
                    production_matrices[f][(p, t)] = total_production

            df = pd.DataFrame.from_dict(production_matrices[f], orient="index", columns=["Value"])
    ...
    def get_demand(self) -> pd.DataFrame:
        ...
        df.index = pd.MultiIndex.from_tuples(df.index, names=["Functional Flow", "Time"])
        df = df.reset_index()
    ...
    def plot_production(self, plot_name: str, dfs_production: pd.DataFrame = None):
        ...
        core_processes = {col.split('_')[2]: col for col in self.m.PROCESS}  # Mapping core name to process
        unique_processes = sorted(set(core_processes.keys()))  # Unique process names (without prefixes)
        colormap = plt.colormaps['tab20']  # A palette with more unique colors
        color_map = {process: colormap(i / len(unique_processes)) for i, process in enumerate(unique_processes)}
        categories = {f: f[:4] for f in self.m.FUNCTIONAL_FLOW}  # Map first four letters to functional flows
        ...
        for ax, f in zip(axes, categories):
            ...
            columns = [col for col in df_production.columns if col.startswith(f"{categories[f]}_prod_")]
