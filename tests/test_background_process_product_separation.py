                ...
                "exchanges": [
                    {
                        ...
                        "input": (
                            "bg_2020",
                            "electricity",
                        ),  # Produces the electricity PRODUCT
                    },
                    {
                        "amount": 0.5,
                        "type": "biosphere",
...
def test_separated_background_process_linkage(setup_separated_background_system):
    ...
    electricity_process = bd.get_node(
        database="bg_2020", name="electricity production process"
    )

    # Verify they are correctly typed
    assert (
        electricity_product.get("type") == bd.labels.product_node_default
    ), "Electricity should be a product node"
    assert (
        electricity_process.get("type") == bd.labels.process_node_default
    ), "Electricity production should be a process node"

    # Verify the process produces the product
    production_exchanges = [
        exc
        for exc in electricity_process.exchanges()
        if exc.get("type") == "production"
    ]

    assert (
        len(production_exchanges) == 1
    ), "Process should have exactly one production exchange"

    assert (
        production_exchanges[0]["input"] == electricity_product.key
