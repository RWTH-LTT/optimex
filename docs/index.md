...
Transitioning towards environmental sustainability requires respecting two fundamental sets of constraints: stock-related limits — such as cumulative carbon budgets and finite mineral resources — and flow-related limits, dictating maximum rates at which resources can be extracted or emissions assimilated. Any viable transition pathway must rigorously respect both.

Designing such pathways demands tools that combine sufficient technological detail with comprehensive environmental coverage. Integrated Assessment Models operate at a macro scale but lack the resolution to identify bottlenecks within specific supply chains or emission profiles. Sector- or process-specific optimization models improve on technological detail, yet often neglect upstream supply chains or environmental impact categories beyond climate change, risking burden shifting. Life Cycle Optimization (LCO) bridges this gap: by embedding Life Cycle Assessment (LCA) within mathematical optimization, it can holistically consider environmental impacts across entire supply chains while *designing* product systems and transition pathways — choosing among alternative processes, supply chain options, and interdependent decisions — rather than just *assessing* a given system.

While several tools and methods exist for LCO, current approaches typically collapse all life cycle stages and emissions to a single point in time. In reality, life cycles span years or decades (*temporal distribution*), and the production systems behind them are continuously evolving (*temporal evolution*). These dimensions are inextricably linked: distribution determines *when* a flow occurs, while evolution determines its *magnitude* at that moment. Together, they dictate whether a pathway stays within stock-related limits — because cumulative impacts depend on the supply chain at the actual time of each exchange — and flow-related limits — because they shift the timing and scale of peak demands and emissions. Ignoring these temporal interdependencies means pathways that appear sustainable on paper may violate environmental constraints when deployed in practice.

`optimex` is a time-explicit LCO framework. It jointly models both temporal dimensions to design product systems and transition pathways that rigorously respect time-specific and cumulative environmental constraints.
