...
Existing LCO tools treat time as a single point, but real life cycles span decades and their supply chains are continuously evolving. These temporal dependencies are critical: ignoring them means pathways that look sustainable on paper may violate environmental limits in practice. `optimex` makes time explicit — jointly modeling *when* flows occur and *how* they change — to design transition pathways that rigorously respect time-specific and cumulative constraints.
