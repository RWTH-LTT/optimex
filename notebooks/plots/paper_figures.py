    ...
    for dtype in ["capacity", "production", "demand", "impacts"]:
        ...
        if filepath.exists():
            df = pd.read_excel(
                filepath,
                index_col=0,
                header=[0, 1] if dtype in ["production", "impacts"] else 0,
            )
            # Clean up index
...
def parse_impacts_df(df: pd.DataFrame) -> dict:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        ...
        for cat in categories:
            if cat in IMPACT_CATEGORIES or any(
                ic in str(cat).lower() for ic in IMPACT_CATEGORIES
            ):
                cat_df = df[cat].copy()
...
def create_combined_results_figure(scenarios_data: dict):
    ...
    for row, (product_key, product_label, is_intermediate) in enumerate(products):
        ...
        for scenario in SCENARIOS.keys():
            ...
            if product_col is not None:
                ...
                additions, retirements = compute_capacity_changes(
                    cap_df[cap_mask], product_col
                )
    ...
    for row, (product_key, product_label, is_intermediate) in enumerate(products):
        for col, (scenario, scenario_label) in enumerate(SCENARIOS.items()):
            ...
            if row == 0 and col == 0:
                ...
                ax = fig.add_subplot(
                    gs[row, col], sharey=axes_matrix[row, 0], sharex=axes_matrix[0, col]
                )
                ...
                if isinstance(c, tuple):
                    ...
                    if product_key.lower() in str(product).lower():
                        ...
                        if clean_process not in process_production:
                            process_production[clean_process] = (
                                prod_df[c].astype(float).values
                            )
                ...
                for process, values in process_production.items():
                    ...
                    if np.any(values_plot > 0.01):
                        ...
                        ax.bar(
                            ...
                            width=bar_width,
                            edgecolor="white",
                            linewidth=linewidth_bar_outline,
                        )
                        bottom += values_plot
    ...
    all_handles.append(
        plt.Line2D([0], [0], color="#000000", linestyle="--", linewidth=1)
    )
    ...
    for row in range(4):  # Changed from range(2) to range(4)
        for col in range(3):
            ax = axes_matrix[
                row, col
            ]  # Changed from axes[row, col] to axes_matrix[row, col]
...
def create_radiative_forcing_figure():
    ...
    for scenario, df in scenario_data.items():
        if not df.empty:
            ...
            cum_max = max(
                cum_max, np.nanmax(cumsum_sum.values) if cumsum_sum.size > 0 else 0
            )
            cum_min = min(
                cum_min, np.nanmin(cumsum_sum.values) if cumsum_sum.size > 0 else 0
            )
    ...
    for col, (scenario, scenario_label) in enumerate(SCENARIOS.items()):
        ...
        for activity in df.columns:
            ...
            ax_inst.plot(
                dates, values_scaled, linewidth=1.2, color=color, label=activity
            )
        ...
        if col == 0:
            ax_inst.set_ylabel(
                f"Instantaneous radiative forcing\n[$10^{{{exponent_inst}}}$ W m$^{{-2}}$]"
            )
        else:
        ...
        stack_data = [
            cumsum_df[a].fillna(0).values / scaling_factor_cum for a in activities_in_df
        ]
        ...
        if col == 0:
            ax_cum.set_ylabel(
                f"Cumulative radiative forcing\n[$10^{{{exponent_cum}}}$ W m$^{{-2}}$]"
            )
        else:
...
def create_combined_forcing_and_impacts_figure(scenarios_data: dict):
    ...
    activity_colors = {
        act: PROCESS_COLORS.get(act, "gray") for act in all_activities_list
    }
    ...
    for scenario in scenarios_data.keys():
        ...
        if "climate_change" in impacts:
            data = impacts["climate_change"].loc[
                (impacts["climate_change"].index >= start_year)
                & (impacts["climate_change"].index <= end_year)
            ]
            crf_bar_max = max(crf_bar_max, data.clip(lower=0).sum(axis=1).max())
            crf_bar_min = min(crf_bar_min, data.clip(upper=0).sum(axis=1).min())
    scaling_factor_crf_bar = (
        10 ** int(np.floor(np.log10(max(abs(crf_bar_max), abs(crf_bar_min)))))
        if max(abs(crf_bar_max), abs(crf_bar_min)) > 0
        else 1
    )
    ...
    scaling_factor_inst = (
        10 ** int(np.floor(np.log10(max(abs(inst_max), abs(inst_min)))))
        if max(abs(inst_max), abs(inst_min)) > 0
        else 1
    )
    ...
    for scenario in scenarios_data.keys():
        ...
        if "water_use" in impacts:
            data = impacts["water_use"].loc[
                (impacts["water_use"].index >= start_year)
                & (impacts["water_use"].index <= end_year)
            ]
            water_max = max(water_max, data.clip(lower=0).sum(axis=1).max())
    scaling_factor_water = (
        10 ** int(np.floor(np.log10(water_max))) if water_max > 0 else 1
    )
    ...
    for col, (scenario, scenario_label) in enumerate(SCENARIOS.items()):
        ...
        if not df_plot.empty:
            y_pos, y_neg = np.zeros(len(full_year_index)), np.zeros(
                len(full_year_index)
            )
        ...
        ax1.set_ylim(
            (inst_min / scaling_factor_inst) * 1.1,
            (inst_max / scaling_factor_inst) * 1.1,
        )
        ax1.set_xticks(xtick_positions)
        ax1.set_xticklabels([])
        if col == 0:
            ax1.set_ylabel(
                f"Instantaneous Radiative\n Forcing[$10^{{{int(np.log10(scaling_factor_inst))}}}$ W m$^{{-2}}$]"
            )
        else:
            ...
            for proc in data.columns:
                ...
                if np.any(vals > 1e-12):
                    ax2.bar(
                        ...
                        vals,
                        bottom=bottom_pos,
                        color=PROCESS_COLORS.get(proc, "gray"),
                        width=0.8,
                        edgecolor="white",
                        linewidth=1,
                    )
    ...
    all_handles = {}
    for act in all_activities_list:
        all_handles[act] = Patch(
            facecolor=activity_colors[act], edgecolor="white", linewidth=0.5
        )
    for scenario in SCENARIOS.keys():
        impacts = load_impacts_properly(scenario)
            ...
            if cat in impacts:
                for proc in impacts[cat].columns:
                    if proc not in all_handles:
                        all_handles[proc] = Patch(
                            facecolor=PROCESS_COLORS.get(proc, "gray"),
                            edgecolor="white",
                            linewidth=0.5,
                        )
...
def create_combined_results_and_impacts_figure(scenarios_data: dict):
    ...
    def add_row_group_label(start_row, end_row, label_text):
        ...
        tick_width = 0.008
        t_top = plt.Line2D(
            [bracket_x, bracket_x + tick_width],
            [y_top, y_top],
            transform=fig.transFigure,
            color="black",
            linewidth=1,
